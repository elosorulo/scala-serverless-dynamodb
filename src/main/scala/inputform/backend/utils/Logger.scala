...
object Logger {
  ...
  def info(output: String): Unit = {
    context match {
      case None =>
      case Some(ctx) => ctx.getLogger.log(s"$INFO_PREFIX$REMAINING_TIME_PREFIX${ctx.getRemainingTimeInMillis}" +
    output +
    "\n")
    }
  }
  ...
  def warn(output: String): Unit = {
    context match {
      case None =>
      case Some(ctx) => ctx.getLogger.log(s"$WARN_PREFIX$REMAINING_TIME_PREFIX${ctx.getRemainingTimeInMillis}" +
        output +
        "\\n")
    }
  }
  ...
  def error(output: String): Unit = {
    context match {
      case None =>
      case Some(ctx) => ctx.getLogger.log(s"$ERROR_PREFIX$REMAINING_TIME_PREFIX${ctx.getRemainingTimeInMillis}" +
        output +
        "\\n")
    }
  }
